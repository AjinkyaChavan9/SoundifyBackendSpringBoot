...
public class PlaylistServiceImpl implements PlaylistService {
		...
		public List<PlaylistResponseDTO> getAllplaylistsByUserId(Long userId) {
			...
			return  playlists.stream()
                    .map(playlist -> mapper.map(playlist,PlaylistResponseDTO.class ))
                    .collect(Collectors.toList());
		}
		@Override
		public List<SongDTO> getAllSongsByPlaylistId(Long playlistId) {
			Playlist playlist = playlistDao.findById(playlistId).orElseThrow(() -> new ResourceNotFoundException("Playlist not dose exists"));
